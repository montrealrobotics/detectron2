...
def getMahaThreshold(prob_val):
    """
        Here, we get probability threshold,
...
class COCOevalLRP:
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        gtm  = np.zeros((T,G))
        dtm  = np.zeros((T,D))
        dtmahadists = np.zeros((T,D))
        gtIg = np.array([g['_ignore'] for g in gt])
        dtIg = np.zeros((T,D))
        if not len(mahadists)==0:
            for dind, d in enumerate(dt):
                # information about best match so far (m=-1 -> unmatched)
                mahadist_thresh = max([p.mahathresh,1e-10])
                m   = -1
                ...
                if m ==-1:
                    continue
                dtIg[0,dind] = gtIg[m]
                dtmahadists[0,dind]= np.exp(-mahadist_thresh / 10.0)
                dtm[0,dind]  = gt[m]['id']
                gtm[0,m]     = d['id']
    ...
    def accumulate(self, p = None):
        ...
        for k, k0 in enumerate(k_list):
            ...
            problocerror = np.squeeze(np.concatenate([e['dtMahaDists'][:,0:p.maxDets]  for e in E], axis=1)[:,inds])
            for i in range(len(problocerror)):
                if problocerror[i]!=0:
                    problocerror[i]=1 - problocerror[i] ## because higher mahadist means less localization error
            ...
            problocerror=np.multiply(problocerror,tps)
            ...
            for s, s0 in enumerate(_pe.confScores):
                ...
                FPError[s,k]=(nhat[s,k]/l)
                FNError[s,k]=(mhat[s,k]/l)
                Z=((omega[s,k]+mhat[s,k]+nhat[s,k])/l);
                LRPError[s,k]=(np.sum(problocerror[:thrind])/l)+FPError[s,k]+FNError[s,k];            
                LRPError[s,k]=LRPError[s,k]/Z;
                LRPError[s,k]=LRPError[s,k];
                LocError[s,k]=np.sum(problocerror[:thrind])/omega[s,k];
        ...
        self.eval = {
            ...
            'LRPError_prob': LRPError,
            'BoxLocComp_prob':  LocError,
            'FPComp_prob': FPError,
            'FNComp_prob': FNError,
            'oLRPError_prob': OptLRPError,
            'oBoxLocComp_prob': OptLocError,
            'oFPComp_prob': OptFPError,
            'oFNComp_prob': OptFNError,
            'moLRP_prob': moLRP,
            'moLRPLoc_prob': moLRPLoc,
            'moLRPFP_prob': moLRPFP,
            'moLRPFN_prob': moLRPFN,
            'OptThresholds_prob':Threshold
        }
        toc = time.time()
        print('DONE (t={:0.2f}s).'.format( toc-tic))
    ...
    def summarize(self, detailed=0):
        ...
        if detailed==1:
            ...
            print('LRP= \n'+str(self.eval['LRPError_prob'])+'\n')
            print('LRPLocalization=\n'+str(self.eval['BoxLocComp_prob'])+'\n')
            print('LRPFalsePositive=\n'+str(self.eval['FPComp_prob'])+'\n')
            print('LRPFalseNegative=\n'+str(self.eval['FNComp_prob'])+'\n')
            print('------------------------------------------------------ \n ')
            print('------------------------------------------------------ \n ')
            print('2.Optimal LRP and Components:')
            print('------------------------------------------------------ \n ')
            print('oLRP='+str(self.eval['oLRPError_prob'])+'\n')
            print('oLRPLocalization=\n'+str(self.eval['oBoxLocComp_prob'])+'\n')
            print('oLRPFalsePositive=\n'+str(self.eval['oFPComp_prob'])+'\n')
            print('oLRPFalseNegative=\n'+str(self.eval['oFNComp_prob'])+'\n')
            print('------------------------------------------------------ \n')
            print('------------------------------------------------------ \n ')
            print('3.Mean Optimal LRP and Components:')
            print('------------------------------------------------------ \n ')
            print('moLRP={:0.4f}, moLRP_LocComp={:0.4f}, moLRP_FPComp={:0.4f}, moLRP_FPComp={:0.4f} \n'.format(self.eval['moLRP_prob'], self.eval['moLRPLoc_prob'],self.eval['moLRPFP_prob'],self.eval['moLRPFN_prob']))
            print('------------------------------------------------------ \n ')
            print('------------------------------------------------------ \n ')
            ...
            print(self.eval['OptThresholds_prob'])
            print('------------------------------------------------------ \n ')
            print('------------------------------------------------------ \n ')
...
class Params:
    ...
    def __init__(self, tau=0.99):
        self.setDetParams()
        self.mahathresh = getMahaThreshold(tau)