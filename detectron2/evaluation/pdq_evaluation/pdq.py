...
def _vectorize_img_gts(gt_instances):
    """
    ...
    gt_coco_boxes = np.stack([gt_instance.coco_bounding_box for gt_instance in gt_instances], axis = 0)   # g x 1
    ...
    return gt_coco_boxes, gt_label_vec


def _vectorize_img_dets(det_instances):
    """
    ...
    """
    det_bboxes_mat = np.stack([det_instance.box for det_instance in det_instances], axis=0) # d x 4
    det_label_prob_mat = np.stack([det_instance.class_list for det_instance in det_instances], axis=0)  # d x c
    det_covars_mat = np.stack([det_instance.covs for det_instance in det_instances], axis=0) # d x 4
    return det_label_prob_mat, det_bboxes_mat, det_covars_mat
...
def _calc_spatial_mahalanobis_qual(det_bboxes_mat, det_covars_mat, gt_coco_boxes):
    """
    calculate the spatial quality with mahalanobis distance. 
    For now, we take scaled likelihood of Gaussian here,
    that would give us value between 0 and 1. 
    :det_bboxes_mat: d x 4 numpy array with coordinates of objects detected(x1, y1, x2, y2)
    :det_covars_mat: d x 4 numpy array with standard deviation values(sigma_x1, sigma_y1, sigma_x2, sigma_y2)
    :gt_coco_boxes: g x 4 numpy array with coordinates of ground truth objects(x1, y1, x2, y2)
    :return: g x d spatial quality score between zero and one for each possible combination of g ground truth objects
    and d detections.
    """

    spatial_quality = np.zeros((gt_coco_boxes.shape[0], det_bboxes_mat.shape[0]))
    for i, g in enumerate(gt_coco_boxes):
        for j, d in enumerate(det_bboxes_mat):

            ## precision matrix
            d_xyxy = np.array([d[0], d[1], d[2], d[3]])
            g_xyxy = np.array([g[0], g[1], g[2], g[3]])
            precision_mat = np.diag(1.0 / det_covars_mat[j]**2) 
            mahadist = np.matmul(np.matmul((d_xyxy - g_xyxy), precision_mat), (d_xyxy - g_xyxy))

            scaled_gaussian_likelihood = np.exp(-mahadist / 10.0)
            spatial_quality[i, j] = scaled_gaussian_likelihood
...
def _gen_cost_tables(gt_instances, det_instances, segment_mode):
    ...
    gt_coco_boxes, gt_label_vec = _vectorize_img_gts(gt_instances)
    det_label_prob_mat, det_bboxes_mat, det_covars_mat = _vectorize_img_dets(det_instances)

    # Calculate spatial and label qualities
    label_qual_mat = _calc_label_qual(gt_label_vec, det_label_prob_mat)
    spatial_qual_mat = _calc_spatial_mahalanobis_qual(det_bboxes_mat, det_covars_mat, gt_coco_boxes)
    ...
    spatial_cost_table[:len(gt_instances), :len(det_instances)] -= spatial_qual_mat
