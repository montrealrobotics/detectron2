...
# sys.path.append('/home/dhaivat1729/pdq_evaluation/LRP/cocoLRPapi-master/PythonAPI/')
        ...
        for img_id, img_dets in enumerate(dets_iter):
            ...
            yield [
                ...
                PBoxDetInst(
                    ...
                    box=det['bbox'],
                    covs=det['sigma'] if self.cov_mat is None else self.cov_mat
                )
                if (self.cov_mat is not None or ('sigma' in det and (np.sum(det['sigma']) != 0)))
...
def read_COCO_gt(filename, n_imgs=None, ret_img_sizes=False, ret_classes=False, bbox_gt=True):
    """
        ...
        for img_idx, img_id in enumerate(img_id_iter):
            if img_id not in coco_annotations.keys():
                ...
                yield [
                    ...
                    for ann_idx in range(len(img_annotations)) 
                ]
...
def convert_coco_det_to_rvc_det(det_filename, gt_filename, save_filename):
    ...
    for img_idx, img_id in enumerate(gt_img_ids):
        ...
        for det_idx, det in enumerate(img_coco_dets):
            ...
            if 'sigma' in det and det['sigma'] is not None:
                covars_vals = det['sigma']
            ...
            det_dict = {'bbox': box, 'sigma': covars_vals, "label_probs": label_probs}
