...
cfg_dict = torch.load('/network/tmp1/bhattdha/detectron2_coco/coco_probabilistic_from_coco_xyxy_deterministic_full_frozen/coco_probabilistic_from_coco_xyxy_deterministic_full_frozen_cfg.final')
...
cfg.OUTPUT_DIR = '/network/tmp1/bhattdha/detectron2_coco/coco_probabilistic_from_coco_xyxy_deterministic_full_frozen/'
...
thresh_dict = np.load('/home/mila/b/bhattdha/detectron2/generative_classifier/maha_thresh_coco.npy',allow_pickle = True)[()]
...
for ind, image_path in enumerate(all_image_paths):
    ...
    cs_metadata = MetadataCatalog.get('coco_2017_val')
    v = Visualizer(im[:, :, ::-1],
    ...
    cv2.imwrite('/network/tmp1/bhattdha/denso_dataset/coco_ood_output/' + str(ind).zfill(6) + '.png', im_new) 
    # import ipdb; ipdb.set_trace()
import ipdb; ipdb.set_trace()